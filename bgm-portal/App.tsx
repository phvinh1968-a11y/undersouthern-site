...
const App: React.FC = () => {
  ...
  const handleDeleteUser = (userId: string) => {
    if (currentUser?.id === userId) {
      alert("Error: Active session cannot be terminated.");
      return;
    }
  ...
  const grantClientRole = (userId: string) => {
    setUsers(prev => prev.map(u => {
        if (u.id === userId) {
            alert(`User ${u.name} has been promoted to CLIENT status.`);
  ...
  return (
    <div className="flex min-h-screen bg-black text-zinc-100 transition-colors duration-500 overflow-x-hidden">
      ...
      <Sidebar 
        ...
        onImportCSV={isElevatedRole ? uploadStatsCSV : undefined}
